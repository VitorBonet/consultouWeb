function generatedata(rowscount, hasNullValues) {
    ...
    for (var i = 0; i < rowscount; i++) {
        ...
        row["id"] = i;
        row["reportsto"] = 9;//Math.floor(Math.random() * firstNames.length);
        // if (i % Math.floor(Math.random() * firstNames.length) === 0) {
        //     row["reportsto"] = null;
        // }

        row["available"] = null;//productindex % 2 == 0;
        ...
        row["firstname"] = 'vitor';//firstNames[Math.floor(Math.random() * firstNames.length)];
        row["lastname"] = 'teste' + lastNames[Math.floor(Math.random() * lastNames.length)];
        row["name"] = row["firstname"] + " " + row["lastname"]; 
        row["productname"] = 4;//productNames[productindex];
        row["price"] = 1;
        row["quantity"] = 1;
        row["total"] = 1 * 1;
